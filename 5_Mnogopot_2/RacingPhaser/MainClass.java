...
import static RacingPhaser.MainClass.CARS_COUNT;
import static RacingPhaser.MainClass.START;

public class MainClass {
    public static final int CARS_COUNT = 4;
    public static final Phaser START = new Phaser(CARS_COUNT);
...
class Car implements Runnable {
    ...
    public void run() {
        ...
        START.awaitAdvance(2);
        for (int i = 0; i < race.getStages().size(); i++) {
            race.getStages().get(i).go(this);
        }
        System.err.println(this.name + " занял " + position.incrementAndGet() + " место");
        START.arrive();
    }
}
...
class Tunnel extends Stage {
    Semaphore TONNEL = new Semaphore(CARS_COUNT / 2);
