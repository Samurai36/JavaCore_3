...
import khlebnikov.viktor.geekbrains.java_two.chat.library.Protocol;
import khlebnikov.viktor.geekbrains.java_two.network.ServerSocketThread;
import khlebnikov.viktor.geekbrains.java_two.network.ServerSocketThreadListener;
...
//            server = new ServerSocketThread(this, "Server", port, 2000);
            server = (ServerSocketThread) executor.submit(new ServerSocketThread(this, "Server", port, 2000));
        }
    }
    ...
    private void putLog(String msg) {
        ...
        try {
            Handler handler = new FileHandler("log_%d.txt", 1024*100, 10, true);
            handler.setFormatter(new Formatter() {

                @Override
                public String format(LogRecord record) {
                    return (System.currentTimeMillis()) + Thread.currentThread().getName() + ": " + record.getMessage();
                }
            });
        } catch (IOException e) {
            e.printStackTrace();
...
//        putLog("Server socket thread accept timeout");
    ...
    public void onSocketAccepted(ServerSocketThread thread, ServerSocket server, Socket socket) {
        ...
        executor.submit(new ClientThread(this, name, socket));
    }
